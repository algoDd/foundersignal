...
class BaseLLMProvider(ABC):
    ...
    async def generate_structured(
        ...
        *,
        ...
        """
        schema_json = json.dumps(_compact_model_spec(response_model), indent=2)
        structured_prompt = (
            f"{prompt}\n\n"
            "IMPORTANT: You MUST return ONLY a raw JSON object containing the "
            "POPULATED DATA based on your analysis.\n"
            "DO NOT output the schema itself. Use the compact field spec below strictly "
            "to structure your resulting JSON data:\n"
            f"```json\n{schema_json}\n```\n"
...
def _compact_model_spec(model_cls: type[BaseModel]) -> dict[str, Any]:
    """Build a compact JSON shape description to reduce prompt-token overhead."""
    return {
        "type": "object",
        "fields": {
            field_name: _describe_annotation(field_info.annotation)
            for field_name, field_info in model_cls.model_fields.items()
        },
    }


def _describe_annotation(annotation: Any) -> Any:
    origin = get_origin(annotation)

    if origin is None:
        if isinstance(annotation, type) and issubclass(annotation, BaseModel):
            return _compact_model_spec(annotation)
    ...
    if origin in (list, tuple):
        ...
        return {"type": "array", "items": _describe_annotation(item_type)}
    ...
    if str(origin).endswith("UnionType") or str(origin).endswith("Union"):
        ...
        if len(args) == 1:
            return {"optional": _describe_annotation(args[0])}
        return {"one_of": [_describe_annotation(arg) for arg in args]}
